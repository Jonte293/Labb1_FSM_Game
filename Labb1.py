    ...
    async def run(self):
        ...
        if choice == "1":
            ...
            if slump < 0.8:
                ...
                print("Ett bra val av klubba, men du slog snett och hamnade i ett svårspelat läge i roughen..")
                self.set_next_state(STATE_ROUGH)
            else:
                print("Wow! Du får en drömträff och landar på green nära flagg!")
...
class Fairway1State(State):
    async def run(self):
        ...
        if choice == "1":
            self.agent.counter += 1
            if slump < 0.85:
                print("Bra val! Du får en bra träff, men landar i bunkern precis innan green..")
                self.set_next_state(STATE_BUNKER)
            elif 0.85 <= slump < 0.99:
                print("Snyggt! Du får en bra träff och landar på green nära flagg!")
                self.set_next_state(STATE_GREEN)
            else:
                print("Wow! Du får en otrolig träff, bollen landar på green och... rullar i hålet!")
                self.set_next_state(STATE_END)
        elif choice == "2":
            self.agent.counter += 1
            if slump < 0.9:
                ...
                print("Bra val, men du träffar snett och hamnar i ett svårt läge i roughen..")
                self.set_next_state(STATE_ROUGH)
        elif choice == "3":
    ...
    async def run(self):
        ...
        if choice == "1":
            ...
            if slump < 0.8:
                ...
                print("Bra val, men tyvärr duffade du och hamnade i bunkern precis innan green.")
                self.set_next_state(STATE_BUNKER)
            else:
                print("Wow! Du får en otrolig träff, bollen landar på green och... rullar i hålet!")
                self.set_next_state(STATE_END)
            ...
            if slump < 0.75:
                ...
                print("Bra val! Bollen studsar på bunkerkanten och landar på green nära flagg!")
                self.set_next_state(STATE_GREEN)
        else:
            print("Felaktig inmatning, försök igen.")
            self.set_next_state(STATE_FAIRWAY_2)
...
class RoughState(State):
    async def run(self):
        ...
        if choice == "1":
            self.agent.counter += 1
            if slump < 0.35:
                print("Bra val! Du slog fram dig bra och landade på fairway med bara 50 meter kvar till green.")
                self.set_next_state(STATE_FAIRWAY_2)
            elif 0.35 <= slump < 0.9:
                print("Bra val, men du hamnade i bunkern precis innan green.")
                self.set_next_state(STATE_BUNKER)
            else:
    ...
    async def run(self):
        ...
        if choice == "1":
            ...
            if slump < 0.9:
                ...
                print("Bra val! Du får perfekt träff, bollen landar på green och... rullar i hålet!")
                self.set_next_state(STATE_END)
        elif choice == "3":
            print("Du slår i kanten och bollen rullar tillbaks ner i bunkern..")
            self.agent.counter += 1
...
class GreenState(State):
    async def run(self):
        ...
        if choice == "1":
            ...
            self.agent.counter += 1
            print("Bra val! Du siktar noggrant och... bollen rullar i hålet!")
            self.set_next_state(STATE_END)
        else:
            print("Felaktig inmatning, försök igen.")
            self.set_next_state(STATE_GREEN)
...
class EndState(State):
    async def run(self):
        ...
        if self.agent.counter == 2:
                print("Eagle!!")
        elif self.agent.counter == 3:
                print("Birdie!!")
...
class Lab1_FSM_golf_Agent(Agent):
    async def setup(self):
        ...
        fsm.add_transition(source=STATE_TEE, dest=STATE_ROUGH)
        fsm.add_transition(source=STATE_TEE, dest=STATE_GREEN)
        ...
        fsm.add_transition(source=STATE_FAIRWAY_1, dest=STATE_ROUGH)
        fsm.add_transition(source=STATE_FAIRWAY_1, dest=STATE_GREEN)
        fsm.add_transition(source=STATE_FAIRWAY_1, dest=STATE_END)
        ...
        fsm.add_transition(source=STATE_FAIRWAY_2, dest=STATE_END)
